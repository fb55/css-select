...
sinkSuite("CSS 1", function(test, ok, b, a, assert) {
    ...
    test("deep messy relationships", 6, function() {
        ...
        assert(
            Q(".direct-descend > .direct-descend .lvl2").length,
            1,
            'found one result for ".direct-descend > .direct-descend .lvl2"'
        );
        assert(
            Q(".direct-descend > .direct-descend div").length,
            1,
            'found one result for ".direct-descend > .direct-descend div"'
        );
        assert(
            Q(".direct-descend > .direct-descend div").length,
            1,
        ...
        assert(
            Q(".direct-descend > .direct-descend > .direct-descend ~ .lvl2").length,
            0,
            "found no results for another odd query"
        );
    });
});
...
sinkSuite("attribute selectors", function(test, ok, b, a, assert) {
    ...
    test("direct descendants", 2, function() {
        ...
        assert(
            Q("#direct-descend > .direct-descend > .lvl2").length,
            3,
            "found three second-level direct descendents"
        );
    });
...
sinkSuite("Uniq", function(test, ok, b, a, assert) {
    test("duplicates arent found in arrays", 2, function() {
        assert(
            Q.uniq(["a", "b", "c", "d", "e", "a", "b", "c", "d", "e"]).length,
            5,
...
sinkSuite("element-context queries", function(test, ok, b, a, assert) {
    test("relationship-first queries", 5, function() {
        var pass = false;
        try {
            pass = Q("> .direct-descend", Q("#direct-descend")).length == 2;
        } catch (e) {}
        ok(pass, "found two direct descendents using > first");

        pass = false;
        try {
            pass = Q("~ .sibling-selector", Q("#sibling-selector")).length == 2;
        } catch (e) {}
        ok(pass, "found two siblings with ~ first");

        pass = false;
        try {
            pass = Q("+ .sibling-selector", Q("#sibling-selector")).length == 1;
        } catch (e) {}
    ...
    frag.innerHTML =
        '<div class="d i v"><p id="oooo"><em></em><em id="emem"></em></p></div><p id="sep"><div class="a"><span></span></div></p>';
    ...
    test("exclude self in match", 1, function() {
        assert(
            Q(".order-matters", Q("#order-matters")).length,
            4,
            "should not include self in element-context queries"
        );
    });
...
sinkSuite("tokenizer", function(test, ok, b, a, assert) {
    test("should not get weird tokens", 5, function() {
        ok(
            Q('div .tokens[title="one"]')[0] == document.getElementById("token-one"),
            'found div .tokens[title="one"]'
        );
        ok(
            Q('div .tokens[title="one two"]')[0] == document.getElementById("token-two"),
            'found div .tokens[title="one two"]'
        );
        ok(
            Q('div .tokens[title="one two three #%"]')[0] == document.getElementById("token-three"),
            'found div .tokens[title="one two three #%"]'
        );
        ok(
            Q("div .tokens[title='one two three #%'] a")[0] == document.getElementById("token-four"),
            "found div .tokens[title='one two three #%'] a"
        );
        ok(
            Q('div .tokens[title="one two three #%"] a[href$=foo] div')[0] ==
                document.getElementById("token-five"),
...
sinkSuite("pseudo-selectors", function(test, ok, b, a, assert) {
    ...
    test(":first-child", 2, function() {
        ok(
            Q("#pseudos div:first-child")[0] ==
                document.getElementById("pseudos").getElementsByTagName("*")[0],
            "found first child"
        );
    ...
    test(":nth-last-child(odd|even|x)", 4, function() {
        ...
        ok(
            Q("#pseudos div:nth-last-child(6)")[0] == second,
    ...
    test(":nth-last-of-type(expr)", 3, function() {
        ...
        ok(
            Q("#pseudos div:nth-last-of-type(5)")[0] == second,
    ...
    test(":first-of-type", 2, function() {
        ok(
            Q("#pseudos a:first-of-type")[0] ==
                document.getElementById("pseudos").getElementsByTagName("a")[0],
...
sinkSuite("argument types", function(test, ok, b, a, assert) {
    ...
    test("should be able to pass in an array of results as arguments", 5, function() {
        ...
        assert(
            Q([Q("#pseudos div.odd"), Q("#pseudos div.even")]).length,
            6,
            "found all the odd and even divs"
        );
    });
});
...
sinkSuite("is()", function(test, ok, b, a, assert) {
    ...
    test("selector sequences combinators", 7, function() {
        ...
        ok(
            Q.is(Q("#token-four")[0], "div#fixtures>div a"),
            "tag#id>tag tag where ambiguous middle tag requires backtracking"
        );
    });
...
sinkSuite("selecting elements in other documents", function(test, ok, b, a, assert) {
    ...
    test("byId sub-queries within sub-context", 6, function() {
        ...
        ok(
            !Q("#booshTest", Q("#spanny", doc)).length,
            "shouldn't find #booshTest (ancestor) within #spanny (descendent)"
        );
        ok(
            !Q("#booshTest", Q("#lonelyHsoob", doc)).length,
            "shouldn't find #booshTest within #lonelyHsoob (unrelated)"
        );
    });
});
