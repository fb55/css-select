...
test("XML Document Selectors", function() {
    ...
    xml = helper.getDOM("<?xml version='1.0' encoding='UTF-8'?><root><elem id='1'/></root>", {
        xmlMode: true
    });
    equal(
...
test("attributes", function() {
    ...
    deepEqual(
        Sizzle("input[data-attr='foo_baz\\']']", null, null, attrbad),
        q("attrbad_injection"),
    ...
    deepEqual(
        Sizzle("input[data-attr='\\\\']", null, null, attrbad),
        q("attrbad_backslash"),
        "Quoted backslash"
    );
    deepEqual(
    ...
    deepEqual(
        Sizzle("input[data-attr='\\5C\\\\']", null, null, attrbad),
        q("attrbad_backslash_backslash"),
    ...
    deepEqual(
        Sizzle("input[data-attr='\\04e00']", null, null, attrbad),
        q("attrbad_unicode"),
...
test("pseudo - has", function() {
    ...
    t("Nested with overlapping candidates", "#qunit-fixture div:has(div:has(div:not([id])))", [
        "moretests",
        "t2037"
    ]);
});
...
test("pseudo - misc", function() {
    ...
    ["button", "submit", "reset"].forEach(function(type) {
        var els = jQuery(
            "<input id='input_%' type='%'/><button id='button_%' type='%'>test</button>".replace(/%/g, type)
        ).appendTo(tmp);
...
test("pseudo - form", function() {
    ...
    var extraTexts = jQuery('<input id="impliedText"/><input id="capitalText" type="TEXT">').appendTo(
        "#form"
    );
