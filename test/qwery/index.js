...
module.exports = {
    Contexts: {
        "should be able to pass optional context"() {
            ...
            expect(
                CSSselect(".a", CSSselect("#boosh", document))
...
*/
        "should not return duplicates from combinators"() {
            ...
            expect(CSSselect("#boosh,.apples,#boosh", document)).to.have.length(
                1
            ); //two booshes and an apple dont make a thing go right
        },
    ...
    "CSS 1": {
        "get element by id"() {
            const result = CSSselect("#boosh", document);
        ...
        "byId sub-queries"() {
            ...
            expect(CSSselect("#boosh>.a>#booshTest", document)).to.have.length(
                1
            ); //found "#id>.class>#id"
        ...
        combos() {
            expect(
                CSSselect("#boosh div,#boosh span", document)
        ...
        "deep messy relationships"() {
            ...
            expect(CSSselect("div#fixtures > div a", document)).to.have.length(
                5
            ); //found four results for "div#fixtures > div a"
            expect(
            ...
            expect(
                CSSselect("div#fixtures div ~ a div", document)
    ...
    "CSS 2": {
        "get elements by attribute"() {
            const [wanted] = CSSselect("#boosh div[test]", document);
            const expected = DomUtils.getElementById("booshTest", document);
            expect(wanted).to.be(expected); //found attribute
            expect(CSSselect("#boosh div[test=fg]", document)[0]).to.be(
                expected
            ); //found attribute with value
            expect(CSSselect('em[rel~="copyright"]', document)).to.have.length(
                1
            ); //found em[rel~="copyright"]
            expect(
                CSSselect('em[nopass~="copyright"]', document)
    ...
    "attribute selectors": {
        ...
        "[attr]"() {
            ...
            expect(
                CSSselect("#attributes div[unique-test]", document)[0]
        ...
        "[attr=val]"() {
            ...
            expect(
                CSSselect('#attributes div[test="two-foo"]', document)[0]
        ...
        "[attr~=val]"() {
            ...
            expect(
                CSSselect("#attributes div[test~=three]", document)[0]
            ).to.be(expected); //found attribute with ~=
        },
        ...
        "[href=#x] special case"() {
            ...
            expect(
                CSSselect('#attributes a[href="#aname"]', document)[0]
        ...
        "sibling elements"() {
            ...
            expect(
                CSSselect(".parent .oldest ~ .sibling", document)
            ).to.have.length(4); //found four younger siblings
            expect(
                CSSselect(".parent .middle ~ .sibling", document)
            ).to.have.length(2); //found two younger siblings
            expect(CSSselect(".parent .middle ~ h4", document)).to.have.length(
                1
            ); //found next sibling by tag
            expect(
            ...
            expect(
                CSSselect(".parent .youngest ~ .sibling", document)
    ...
    /*
'Uniq': {
	'duplicates arent found in arrays': () => {
...
*/
    ...
    "element-context queries": {
        "relationship-first queries"() {
            expect(
                CSSselect(
                    "> .direct-descend",
                    CSSselect("#direct-descend", document)
                )
        ...
        "byId sub-queries within detached fragment"() {
            ...
            expect(
                CSSselect("#sep", CSSselect("#emem", frag)).length
        ...
        "exclude self in match"() {
            expect(
                CSSselect(
                    ".order-matters",
                    CSSselect("#order-matters", document)[0]
                )
        ...
        "forms can be used as contexts"() {
            expect(
                CSSselect("*", CSSselect("form", document)[0])
    ...
    tokenizer: {
        "should not get weird tokens"() {
            ...
            expect(
                CSSselect('div .tokens[title="one two"]', document)[0]
    ...
    "order matters": {
        ...
        "the order of elements return matters"() {
            ...
            const els = CSSselect("#order-matters .order-matters", document);
    ...
    "pseudo-selectors": {
        ":contains"() {
            expect(CSSselect("li:contains(humans)", document)).to.have.length(
                1
            ); //found by "element:contains(text)"
            ...
            expect(CSSselect("*:contains(humans)", document)).to.have.length(5); //found by "*:contains(text)", including all ancestors
            expect(CSSselect("ol:contains(humans)", document)).to.have.length(
                1
            ); //found by "ancestor:contains(text)"
        },
        ...
        'ol > li[attr="boosh"]:last-child'() {
            ...
            expect(
                CSSselect('ol > li[attr="boosh"]:last-child', document)[0]
        ...
        ":nth-child(odd|even|x)"() {
            ...
            expect(
                CSSselect("#pseudos :nth-child(odd)", document)
            ).to.have.length(4); //found 4 odd elements
            expect(
                CSSselect("#pseudos div:nth-child(odd)", document)
            ...
            expect(CSSselect("#pseudos div:nth-child(2)", document)[0]).to.be(
                second
            ); //found 2nd nth-child of pseudos
        },
        ...
        ":nth-child(expr)"() {
            ...
            expect(
                CSSselect("#pseudos :nth-child(3n+1)", document)
            ).to.have.length(3); //found 3 elements
            expect(
                CSSselect("#pseudos :nth-child(+3n-2)", document)
            ).to.have.length(3); //found 3 elements'
            expect(
                CSSselect("#pseudos :nth-child(-n+6)", document)
            ).to.have.length(6); //found 6 elements
            expect(
                CSSselect("#pseudos :nth-child(-n+5)", document)
            ).to.have.length(5); //found 5 elements
            expect(CSSselect("#pseudos :nth-child(3n+2)", document)[1]).to.be(
                fifth
            ); //second :nth-child(3n+2) is the fifth child
            expect(CSSselect("#pseudos :nth-child(3n)", document)[1]).to.be(
                sixth
            ); //second :nth-child(3n) is the sixth child
        },
        ...
        ":nth-last-child(odd|even|x)"() {
            ...
            expect(
                CSSselect("#pseudos div:nth-last-child(6)", document)[0]
        ...
        ":nth-last-child(expr)"() {
            ...
            expect(
                CSSselect("#pseudos :nth-last-child(3n+2)", document)[0]
        ...
        ":nth-of-type(expr)"() {
            ...
            expect(
                CSSselect("#pseudos a:nth-of-type(3n+1)", document)[0]
            ).to.be(a); //found the right a element
            expect(
                CSSselect("#pseudos a:nth-of-type(3n)", document)
            ).to.be.empty(); //no matches for every third a
            expect(
                CSSselect("#pseudos a:nth-of-type(odd)", document)
            ).to.have.length(1); //found the odd a
            expect(
                CSSselect("#pseudos a:nth-of-type(1)", document)
        ...
        ":nth-last-of-type(expr)"() {
            ...
            expect(
                CSSselect("#pseudos div:nth-last-of-type(5)", document)[0]
        ...
        ":first-of-type"() {
            ...
            expect(
                CSSselect("#pseudos a:first-of-type", document)
            ).to.have.length(1); //found only 1
        },
        ...
        ":last-of-type"() {
            ...
            expect(
                CSSselect("#pseudos div:last-of-type", document)
            ).to.have.length(1); //found only 1
        },
...
*/
    ...
    "selecting elements in other documents": {
        "get element by id"() {
            const result = CSSselect("#hsoob", doc);
        ...
        "complex selectors"() {
            ...
            expect(CSSselect('.a .d ~ .sib[test="f g"]', doc)).to.have.length(
                1
            ); //found 1 ~ sibling with test attribute
        },
        ...
        "byId sub-queries within sub-context"() {
            expect(
                CSSselect("#spanny", CSSselect("#hsoob", doc))
            ).to.have.length(1); //found "#id -> #id" in frame
            expect(
                CSSselect(".a #spanny", CSSselect("#hsoob", doc))
