...
function getAttribFunc(name, value){
	...
	return function attribFunc(next){
		return checkAttrib(next, data, this);
	};
}
...
var filters = {
	...
	scope: function(next, rule, context){
		var adapter = this.adapter;

		if(!context || context.length === 0){
			//equivalent to :root
			return filters.root.call(this, next);
		}
...
var pseudos = {
	...
	link: function(elem){
		return this.adapter.hasAttrib(elem, "href");
	},
	...
	disabled: function(elem){
		return this.adapter.hasAttrib(elem, "disabled");
	},
	...
	checked: function(elem){
		return (
			this.adapter.hasAttrib(elem, "checked") || pseudos.selected.call(this, elem)
		);
	},
	//:matches(input, select, textarea)[required]
	required: function(elem){
		return this.adapter.hasAttrib(elem, "required");
	},
	...
	parent: function(elem){
		return !pseudos.empty.call(this, elem);
	},
...
function namePseudo(names){
	if(typeof Set !== "undefined"){
		...
		return function(elem){
			return nameSet.has(this.adapter.getName(elem));
		};
	}

	return function(elem){
		return names.indexOf(this.adapter.getName(elem)) >= 0;
	};
}
...
module.exports = {
	compile: function(next, data, options, context){
		...
		if(typeof filters[name] === "function"){
			return filters[name].call(options, next, subselect, context);
			...
			if(next === trueFunc){
				return function pseudoRoot(elem){
					return func.call(options, elem, subselect);
				};
			}

			return function pseudoArgs(elem){
				return func.call(options, elem, subselect) && next(elem);
			};
		} else {
